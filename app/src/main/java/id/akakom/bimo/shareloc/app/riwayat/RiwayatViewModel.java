...
public class RiwayatViewModel extends BaseViewModel {

    MutableLiveData isSuccess = new MutableLiveData<ArrayList<Shareloc>>();

    public RiwayatViewModel(@NotNull Context context) {
        super(context);
    }

    void get(){
        GmsRequest.POST(EndPoints.stringAllRiwayat(), getContext(), new GmsRequest.OnPostRequest() {
            @Override
            ...
            public void onSuccess(JSONObject response) {
                ...
                try {
                    ...
                    if (response.getInt("status") == 200) {
                        Type listType = new TypeToken<ArrayList<Shareloc>>() {}.getType();
                        ArrayList<Shareloc> sharelocs = new Gson().fromJson(response.getString("data"), listType);
                        isSuccess.postValue(sharelocs);
                    } else {
                        isSuccess.postValue(new ArrayList<Shareloc>());
                    }
